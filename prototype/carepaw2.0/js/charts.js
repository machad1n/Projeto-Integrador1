...
function createTrendsChart() {
    ...
    const popSeries = years.map(y=>{ 
        const pop = (dataset.find(d=> d.Ano===y && d.Categoria && d.Categoria.indexOf('População Pet (Lares)')>=0) || {}).Valor || 0;
        return pop;
    }).filter(v=>v>0);
    const vulnSeries = years.map(y=>{ 
        const vuln = (dataset.find(d=> d.Ano===y && d.Categoria && d.Categoria.indexOf('Vulnerabilidade Pet')>=0) || {}).Valor || 0;
        return vuln;
    }).filter(v=>v>0);
    const trendYears = years.filter(y=>{
        const pop = (dataset.find(d=> d.Ano===y && d.Categoria && d.Categoria.indexOf('População Pet (Lares)')>=0) || {}).Valor || 0;
        const vuln = (dataset.find(d=> d.Ano===y && d.Categoria && d.Categoria.indexOf('Vulnerabilidade Pet')>=0) || {}).Valor || 0;
        return pop > 0 || vuln > 0;
    });
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'Tendência Populacional e Vulnerabilidade (Amostral)',
                    font: {
                        family: 'Poppins',
                        size: 14,
                        weight: '600'
